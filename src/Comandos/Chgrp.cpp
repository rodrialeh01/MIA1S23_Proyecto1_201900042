...
void Chgrp::CambiarGrupo(Chgrp*chgrp){
    string user = chgrp->user;
    string group = chgrp->group;
    if(user == ""){
    ...
    if(!ExisteUsuario(user)){
        cout << "\e[1;31m[ERROR]:\e[1;37m El nombre del usuario no existe \e[m\n" << endl;
        return;
    }
    ...
    for(int i = 0; i < usuarios_grupos.size(); i++){
        ...
        if(datos[1] == "U"){
            if(datos[3] == user){
                nuevo_txt += datos[0] + "," + group + "," + datos[2] + "," + datos[3] + "," + datos[4] + "\n";
                encontrado = true;
            }else{
    ...
    cout << "\e[1;32m [SUCCESS]: \e[1;37m Se ha modificado el grupo exitosamente \e[m\n" << endl;
    

}

vector<EBR> Chgrp::ListadoEBRS(Particion extendida, string path){
    vector<EBR> ebrs;
    FILE *archivo;
...
bool Chgrp::cadenaVacia(char cadena[]){
    for(int i = 0; i < 16; i++) {
        if(cadena[i] != '\0') {
            return false; 
        }
...
vector<string> Chgrp::split(string texto, char parametro) {
...
string Chgrp::toLowerCase(string palabra){
    int i = 0;
    string retorno = "";
...
bool Chgrp::ExisteUsuario(string name){
    //OBTIENE LA PARTICION
    Nodo particion = lista_particiones_montadas.obtenerNodoParticion(idlogueado);
...
int Chgrp::tamanio_disponible(BloqueArchivo contenido){
    int contador = 0;
    for(int i = 0; i < 63; i++){
...
bool Chgrp::ExisteGrupo(string grupo){
    //OBTIENE LA PARTICION
    Nodo particion = lista_particiones_montadas.obtenerNodoParticion(idlogueado);
