...
void Mkusr::CrearUsuario(Mkusr*mkusr){
    string user = mkusr->user;
    string password = mkusr->password;
    string group = mkusr->group;
    if(user == ""){
    ...
    if(password == ""){
        cout << "\e[1;31m[ERROR]:\e[1;37m No ha agregado la contraseña del grupo \e[m\n" << endl;
        return;
    }

    if(group == ""){
        cout << "\e[1;31m[ERROR]:\e[1;37m No ha agregado el grupo al que pertenece \e[m\n" << endl;
        return;
    }
    ...
    if(userlogueado != "root"){
        cout << "\e[1;31m[ERROR]:\e[1;37m NO se cuenta con los permisos necesarios para poder crear usuarios \e[m\n" << endl;
        return;
    }
    ...
    if(group[0]== '\"' && group[group.size()-1] == '\"'){
        group = group.substr(1, group.size()-2);
    }

    if(user.size() > 10){
        cout << "\e[1;31m[ERROR]:\e[1;37m El nombre del usuario es muy largo \e[m\n" << endl;
        return;
    }

    if(password.size() > 10){
        cout << "\e[1;31m[ERROR]:\e[1;37m La contraseña es muy larga \e[m\n" << endl;
        return;
    }
    ...
    if(ExisteUsuario(user)){
        cout << "\e[1;31m[ERROR]:\e[1;37m El nombre del usuario ya existe \e[m\n" << endl;
        return;
    }

    if(!ExisteGrupo(group)){
        cout << "\e[1;31m[ERROR]:\e[1;37m El grupo no existe \e[m\n" << endl;
        return;
    }

    CreateUsuario(user, password, group);

}

vector<EBR> Mkusr::ListadoEBRS(Particion extendida, string path){
    vector<EBR> ebrs;
    FILE *archivo;
...
bool Mkusr::cadenaVacia(char cadena[]){
    for(int i = 0; i < 16; i++) {
        if(cadena[i] != '\0') {
            return false; 
        }
...
vector<string> Mkusr::split(string texto, char parametro) {
...
string Mkusr::toLowerCase(string palabra){
    int i = 0;
    string retorno = "";
...
void Mkusr::CreateUsuario(string name, string password, string group){
    ...
    string linea_usuario = to_string(NumeroUsuario()) + ",U," + group + "," + name + "," + password + "\n";
    for(int i = 0; i < 12; i++){
        if(i != 12){
            ...
            if(no_bloque != -1){
                if(inodo.i_block[i+1] == -1){
                    ...
                    int espacio = tamanio_disponible(bloque)- linea_usuario.size();
                    if(espacio >= 0){
                        contenido += linea_usuario;
                        ...
                        if(tamanio_disponible(bloque) == 0){
                            ...
                            BloqueArchivo nuevo;
                            strcpy(nuevo.b_content, linea_usuario.c_str());
                            ...
                            break;
                        }else if(tamanio_disponible(bloque) !=0){
                            string parte1 = linea_usuario.substr(0, tamanio_disponible(bloque));
                            string parte2 = linea_usuario.substr(tamanio_disponible(bloque), linea_usuario.length());
                            contenido += parte1;
            ...
            fread(&bloque, sizeof(BloqueArchivo), 1, archivo);
            string contenido = string(bloque.b_content);
            int espacio = tamanio_disponible(bloque)- linea_usuario.length();
            if(espacio >= 0){
                contenido += linea_usuario;
                strcpy(bloque.b_content, contenido.c_str());
                fseek(archivo, no_bloque, SEEK_SET);
    ...
    cout << "\e[1;32m [SUCCESS]: \e[1;37m Se ha creado el usuario: " << name << " exitosamente \e[m\n" << endl;

}

bool Mkusr::ExisteUsuario(string name){
    //OBTIENE LA PARTICION
    Nodo particion = lista_particiones_montadas.obtenerNodoParticion(idlogueado);
    ...
    for(int i = 0; i < usuarios_grupos.size(); i++){
        ...
        if(datos[1] == "U" && datos[3] == name){
            return true;
        }else if(datos[0] == "0" && datos[1] == "U" && datos[3] == name){
            return false;
        }
    }
    return false;
...
bool Mkusr::ExisteGrupo(string grupo){
    //OBTIENE LA PARTICION
    Nodo particion = lista_particiones_montadas.obtenerNodoParticion(idlogueado);
    ...
    for(int i = 0; i < usuarios_grupos.size(); i++){
        vector<string> datos;
        datos = split(usuarios_grupos[i], ',');
        if(datos[1] == "G" && datos[2] == grupo){
            return true;
        }else if(datos[0] == "0" && datos[1] == "G" && datos[2] == grupo){
            return false;
        }
    }
    return false;
...
int Mkusr::tamanio_disponible(BloqueArchivo contenido){
    int contador = 0;
    for(int i = 0; i < 63; i++){
