...
void Rmusr::EliminarUsuario(Rmusr*rmusr){
    string user = rmusr->user;
    if(user == ""){
...
vector<EBR> Rmusr::ListadoEBRS(Particion extendida, string path){
    vector<EBR> ebrs;
    FILE *archivo;
...
bool Rmusr::cadenaVacia(char cadena[]){
    for(int i = 0; i < 16; i++) {
        if(cadena[i] != '\0') {
            return false; 
        }
...
vector<string> Rmusr::split(string texto, char parametro) {
...
string Rmusr::toLowerCase(string palabra){
    int i = 0;
    string retorno = "";
...
void Rmusr::DeleteUsuario(string user){
    ...
    for(int i = 0; i < usuarios_grupos.size(); i++){
        ...
        if(datos[1] == "U"){
            if(datos[3] == user){
                nuevo_txt += "0," + datos[1] + "," + datos[2] + "," + datos[3] + "," + datos[4] + "\n";
                encontrado = true;
            }else{
    ...
    if(encontrado == false){
        cout << "\e[1;31m[ERROR]:\e[1;37m No existe el usuario: " << user << "\e[m\n" << endl;
        return;
    }
    ...
    cout << "\e[1;32m [SUCCESS]: \e[1;37m Se ha eliminado el usuario exitosamente \e[m\n" << endl;

}

bool Rmusr::ExisteUsuario(string name){
    //OBTIENE LA PARTICION
    Nodo particion = lista_particiones_montadas.obtenerNodoParticion(idlogueado);
...
int Rmusr::tamanio_disponible(BloqueArchivo contenido){
    int contador = 0;
    for(int i = 0; i < 63; i++){
